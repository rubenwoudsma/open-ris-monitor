...
export function getDocumentIdentifiers(documentRecord) {
...
export function getRelationDocumentIdentifiers(relation) {
    const identifiers = new Set();
...
function resolveDocumentId(indexes, relation) {
    for (const identifier of getRelationDocumentIdentifiers(relation)) {
        if (indexes.documentsById.has(identifier))
            return identifier;
    }
    return pick(relation.document_id, relation.document_source_id, relation.document_object_id);
}
export function buildRelationIndexes(documents, meetings, agendaItems, meetingDocumentRelations, meetingItemDocumentRelations) {
    const indexes = {
